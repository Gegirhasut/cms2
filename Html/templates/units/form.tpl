    ...
    <div class="col-md-12">
            ...
            {foreach from=$form.model->fields item=field key=key}
                {if !isset($field.nolist)}
                    {if $field.type eq 'hidden'}
                        {include file='units/fields/hidden.tpl'}
                    {elseif $field.type eq 'yandexmap'}
        ...
        <div class="form-group">
            <div class="col-sm-offset-{$form.label_width}">
                <button type="submit" class="btn btn-info" id="{if isset($form.id)}{$form.id}{else}form_btn{/if}" >{$form.action_name}</button>
            </div>
        </div>
    </div>
</div>